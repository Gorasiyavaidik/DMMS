...
const Polishing = (data) => {
    const navigate = useNavigate();
    const [open , setOpen] = useState(false);
    const [serialNo, setSerialNo] = useState("");
    const handleChangeState = () =>{
        serialNo && navigate(`/inventory/changestate/${serialNo}` , {state : {state : "Polishing"}});
    }
    return (
            ...
            <tbody>
                ...
                <tr key={i}>
                    ...
                    <div className="flex items-center space-x-3.5">
                        <button
                        onClick={() => { setSerialNo(item.serialNo) ; setOpen(true)}}
        ...
        <Modal open={open} onClose={() => setOpen(false)}>
            ...
            {/* <Link to={`/inventory/editinventory/${serialNo}`} >
                ...
                <button className="inline-flex items-center justify-center rounded-md border border-primary py-1 px-4 mx-1 text-center font-medium text-white bg-primary hover:bg-opacity-90 lg:px-8 xl:px-8">Delete</button>
                <button onClick={handleChangeState} className="inline-flex items-center justify-center rounded-md border border-primary py-1 px-4 mx-1 text-center font-medium text-white bg-primary hover:bg-opacity-90 lg:px-8 xl:px-8">Move to Polished</button>
            </div>
            </div>
        </Modal>
        </div>
