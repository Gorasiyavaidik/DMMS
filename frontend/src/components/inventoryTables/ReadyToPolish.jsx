...
const ReadyToPolish = (data) => {
    const navigate = useNavigate();
    const [open , setOpen] = useState(false);
    
    const [serialNo, setSerialNo] = useState("");
    const handleChangeState = () =>{
        serialNo && navigate(`/inventory/changestate/${serialNo}` , {state : {state : "ReadyToPolish"}});
    }
    ...
    return (
        ...
        <Modal open={open} onClose={() => setOpen(false)}>
            ...
            {/* <button className="inline-flex items-center justify-center rounded-md border border-primary py-1 px-4 mx-1 text-center font-medium text-white bg-primary hover:bg-opacity-90 lg:px-8 xl:px-8">Edit</button> */}
                ...
                <button onClick={handleChangeState} className="inline-flex items-center justify-center rounded-md border border-primary py-1 px-4 mx-1 text-center font-medium text-white bg-primary hover:bg-opacity-90 lg:px-8 xl:px-8">Assign To Workshop</button>
            </div>
            </div>
        </Modal>
        </div>
