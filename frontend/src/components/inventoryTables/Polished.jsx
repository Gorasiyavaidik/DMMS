...
const Polished = (data) => {
    ...
    const handleChangeState = () =>{
        serialNo && navigate(`/inventory/selldiamond` , {state : {state : "Polishing"}});
    }
    ...
    return (
        ...
        <Modal open={open} onClose={() => setOpen(false)}>
            ...
            {/* <Link to={`/inventory/editinventory/${serialNo}`} >
                ...
                <button onClick={handleChangeState} className="inline-flex items-center justify-center rounded-md border border-primary py-1 px-4 mx-1 text-center font-medium text-white bg-primary hover:bg-opacity-90 lg:px-8 xl:px-8">Add Sold Details</button>
            </div>
            </div>
        </Modal>
        </div>
